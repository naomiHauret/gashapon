  ...
  async function setDefaultUserProfile(defaultAccountId) {
    ...
    try {
      ...
      if (newDefaultProfile?.data) {
        ...
        toast().create({
          type: 'error',
          title: `Something went wrong and your new default profile couldn't be set: ${newDefaultProfile.error.message}`,
        })
      }
      ...
      toast().create({
        type: 'error',
        title: `Something went wrong and your new default profile couldn't be set: ${e?.message ?? e}`,
      })
      console.error(e)
    }
  }
