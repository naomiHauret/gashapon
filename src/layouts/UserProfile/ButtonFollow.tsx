    ...
    <>
                    ...
                    disabled={
                      ...
                      (props?.profile?.followModule?.type === 'FeeFollowModule' &&
                        (balanceState.loading ||
                          parseFloat(
                            balanceState?.balanceOf?.[props?.profile?.followModule?.amount?.asset?.address]
                              ?.formatted,
                          ) < parseFloat(props?.profile?.followModule?.amount?.value))) ||
                      (!stateFetchDefaultProfile?.data?.id &&
                        props?.profile?.followModule?.type === 'ProfileFollowModule')
                    }
                    isLoading={stateFollowRequest.isLoading === true || stateUnfollowRequest.isLoading}
                    onClick={async () => {
                      if (
                        stateIsFollowingThisProfile.data !== null &&
                        stateIsFollowingThisProfile?.data[0]?.follows === true
                      ) {
                    ...
                    <Switch>
                      ...
                      >
                        <Switch fallback="Unfollow">
                          <Match when={!stateUnfollowRequest.isLoading && !stateUnfollowRequest.isError}>
                            Unfollow
                          </Match>
                  ...
                  >
                    <div class="text-[0.765rem] text-white text-opacity-50 mt-1">
                      <Switch>
                        <Match when={props?.profile?.followModule === null}>
                          Following {props?.profile?.name} is free
                        </Match>
                        ...
                        <Match when={props?.profile?.followModule?.type === 'FeeFollowModule'}>
                          ...
                          <span class="font-bold">
                            {props?.profile?.followModule?.amount?.value}{' '}
                            {props?.profile?.followModule?.amount?.asset?.symbol}
                          </span>
                        </Match>
